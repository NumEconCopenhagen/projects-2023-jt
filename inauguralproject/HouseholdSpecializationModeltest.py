...
class HouseholdSpecializationModelClassExt:
    ...
    def calc_utility(self, LM, HM, LF, HF):
        ...
        epsilon_f = 1 + 1 / par.epsilonf  # Update epsilon_f value
    ...
    def estimate(self, sigma=None, do_print=False):
        ...
        epsilonf_guess = 2.3
        sigma_guess = 1
        es_guess = (epsilonf_guess, sigma_guess)
        ...
        def obj(x):
            ...
            self.solve_wF_vec()
            self.run_regression()  # Update epsilonf and sigma before running regression
            Rsqr = (par.beta0_target - sol.beta0) ** 2 + (par.beta1_target - sol.beta1) ** 2
            return Rsqr

        # Minimize the R-squared value with scipy
        goal = optimize.minimize(obj, es_guess, method="Nelder-Mead", bounds=bounds)

        # Update epsilon_f and sigma with the optimized values
        par.epsilonf = goal.x[0]
        par.sigma = goal.x[1]

        # Print the optimized values if required
        if do_print:
            self.solve_wF_vec()
            self.run_regression()
            Rsqr = (par.beta0_target - sol.beta0) ** 2 + (par.beta1_target - sol.beta1) ** 2
